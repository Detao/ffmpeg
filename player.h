...
typedef struct VideoState
{
    ...
    double          video_clock;//最后解码的那一帧的pts
    double          video_current_pts;//当前播放的pts
    int64_t         video_current_pts_time;//time (av_gettime)更新video_current_pts时的时间
...
//添加或减少samples来更好的同步，返回一个新的音频缓存的大小
int synchronize_audio(VideoState *is, short *samples,
    ...
    if(is->av_sync_type != AV_SYNC_AUDIO_MASTER){//不是以音频时钟来同步
        double diff, avg_diff;
        int wanted_size, min_size, max_size;

        ref_clock = get_master_clock(is);
        diff = get_audio_clock(is) - ref_clock;//与主时钟对比
